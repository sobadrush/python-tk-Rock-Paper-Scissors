...
class GAMING_INTERFACE:
    ...
    def run(self):
        ...
        ####### END #######
        ####### robot_frame #######
        self.robot_frame, self.robot_scissors_label, self.robot_stone_label, \
              self.robot_paper_label, self.robot_points_var = self.__set_robot_frame(root) # TODO: 設定電腦的畫面
        # TODO: 放置電腦的畫面
        self.robot_frame.place( x=self.GAME_WIDTH*0.5, y=self.GAME_HEIGHT*0.2 ) # 放置電腦的畫面
        ####### END #######

        ####### result_frame #######
        self.result_frame, self.result = self.__set_result_frame(root) # TODO: 設定結果的畫面
        ####### END #######
    ...
    def __load_img(self): # 載入圖片
        ...
        self.scissors_img = PhotoImage( file=self.SCISSORS_IMG_PATH).zoom(13).subsample(30)#載入剪刀圖片
        self.stone_img = PhotoImage( file=self.STONE_IMG_PATH ).zoom(13).subsample(30)#載入石頭圖片
        self.paper_img = PhotoImage( file=self.PAPER_IMG_PATH ).zoom(13).subsample(30)#載入布圖片
    ...
    def __set_title_frame(self, root): # 設定標題
        ...
        studentNameLable = Label( title_frame, bg='lightgray', text=f'{self.STUDENT_NAME}', font=('Microsoft JhengHei', 20), fg='darkblue' ) # 設定學生姓名
        studentNameLable.place( x=self.GAME_WIDTH * 0.05, y=10, anchor='n' ) # 放置學生姓名
    ...
    def __set_user_frame(self,root):
        ...
        user_stone_btn.place( x=340, y=400 ,anchor='center')# 放置石頭按鈕
        ...
        user_paper_btn.place( x=540, y=400 ,anchor='center')# 放置布按鈕
    ...
    def __click_stone(self):
        ...
        if self.user_scissors_btn['state']!='normal' or self.user_paper_btn['state']!='normal' or self.user_stone_btn['state']!='normal':
            ...
            self.user_stone_btn['state']=DISABLED
    ...
    def __click_paper(self):
        ...
        if self.user_scissors_btn['state']!='normal' or self.user_paper_btn['state']!='normal' or self.user_stone_btn['state']!='normal':
            ...
            self.user_paper_btn['state']=DISABLED
    ...
    def __set_robot_frame(self,root):
        ...
        robot_stone_label = Label( robot_frame, image=self.stone_img, text='stone', background=self.ROBOT_BUTTON_COLOR ) # TODO: 設定電腦的石頭
        # TODO: 放置電腦的石頭
        robot_stone_label.place( x=360, y=400, anchor='center' ) # 放置電腦的石頭

        robot_paper_label = Label( robot_frame, image=self.paper_img, text='paper', background=self.ROBOT_BUTTON_COLOR ) # TODO: 設定電腦的布
        # TODO: 放置電腦的布
        ...
        robot_points_label = Label( robot_frame, bg=self.USER_FRAME_BACKGROUNG, \
          textvariable=robot_points_var, font=('Arial',30), fg='black' ) # TODO: 設定電腦的分數
        # TODO: 放置電腦的分數
        robot_points_label.place( x=self.GAME_WIDTH*0.75, y=250, anchor='n' )
    ...
    def __set_result_frame(self,root):
        ...
        result_label.place( x=250, y=120, anchor='center' ) # 放置結果
        
        # 設定重置按鈕
        reset_btn = Button( result_frame, text='RESET', font=('Arial',12), command=self.__reset_func, relief='groove', background=self.RESET_BUTTON_DARK_COLOR, activebackground=self.RESET_BUTTON_LIGHT_COLOR, fg=self.RESET_BUTTON_LIGHT_COLOR, activeforeground=self.RESET_BUTTON_DARK_COLOR )
        # reset_btn = Button( result_frame, text='RESET', font=('Arial',12), command=lambda: None, relief='groove', background=self.RESET_BUTTON_DARK_COLOR, activebackground=self.RESET_BUTTON_LIGHT_COLOR, fg=self.RESET_BUTTON_LIGHT_COLOR, activeforeground=self.RESET_BUTTON_DARK_COLOR )
        
        # 設定重置按鈕的hover效果
        self.__changeOnHover(reset_btn, self.RESET_BUTTON_LIGHT_COLOR, self.RESET_BUTTON_DARK_COLOR)
        
        # 放置重置按鈕
        reset_btn.place( x=250, y=190, anchor='center' )
    ...
    def __update_scoreboard(self):
        ...
        # TODO: 根據 user_points 以 for 迴圈更新 user_score: ★


        # TODO: 根據 robot_points 以 for 迴圈更新 robot_score: ★
